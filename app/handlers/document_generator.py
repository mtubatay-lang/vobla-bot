...
# На шаге «реквизиты» из одного ввода извлекаем поля через AI/Vision
REQUISITES_FIELD_SPECS = [
    {"key": "customer_name", "label": "ФИО или название ИП/ООО Заказчика"},
    {
        "key": "customer_preamble",
        "label": "Для преамбулы договора: если ИП — «Индивидуальный предприниматель ФИО», если ООО — «Общество с ограниченной ответственностью «Название»» (не использовать «Индивидуальный предприниматель» для ООО)",
    },
...
async def doc_gen_upload_callback(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.set_state(TemplateUploadState.waiting_file)
    await cb.message.answer(
        "📤 Отправьте DOCX-файл с плейсхолдерами в формате {{field_key}} (например {{contract_number}}, {{contract_date}}, {{customer_name}}, {{customer_requisites}}).\n\nДля отмены: /cancel",
        parse_mode=ParseMode.HTML,
    )
    await cb.answer()
