...
_NOT_QUESTION_PHRASES = frozenset(
    s.lower() for s in ("привет", "спасибо", "благодарю", "готово", "добрый день", "здравствуйте", "проверьте пожалуйста")
)
...
async def _is_question(message_text: str) -> bool:
    ...
    try:
        prompt = (
            ...
            "Если в сообщении есть тема из базы (регламент, документ, касса, открытие, ИП, ЕГАИС и т.п.) и это не приветствие/благодарность — ответь yes.\n"
            "Ответь только 'yes' или 'no'.\n\n"
            f"Сообщение: {text}"
        )
        system_content = (
            "Ты помощник. Определяешь, ждёт ли пользователь ответа от базы знаний/поддержки. "
            "Вопрос может быть без «?». Короткие формулировки по темам регламент, документы, касса, открытие, ИП, ЕГАИС — всегда считай вопросом (yes), даже без вопросительного знака. "
            "Пример: «Регламент по открытию магазина» без знака вопроса = yes. "
            "Формулировки «Нужна информация…», «Подскажите…», «Хочу уточнить…» — тоже запросы к базе. Отвечай только 'yes' или 'no'."
        )
