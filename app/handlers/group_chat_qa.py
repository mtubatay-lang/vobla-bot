...
_NOT_QUESTION_PHRASES = frozenset(
    ...
    for s in (
        ...
        "доброго времени",
        "как обстановка",
        "можете контакты направить",
    )
)

# Паттерны для «утверждение / статус» (не вопрос)
_STATEMENT_PATTERNS = (
    r"\b(пришлю|перешлю|отправлю|отправил)\b",
    r"\bжду\s+",  # «жду кассовую стойку»
    r"\b(забрал|получил)\b",
    r"\bне\s+(связывали(сь)?|связали(сь)?)\b",
    r"^доброго\s+времени\b",
    r"\bкак\s+обстановка\b",
    r"\bминуту\s+через\b",  # «минуту через 5»
)


def _is_statement_or_status(text: str) -> bool:
    """True если сообщение — утверждение о своём действии или статус ожидания, а не вопрос."""
    if not text or len(text.strip()) < 5:
        return False
    lower = text.strip().lower()
    return any(re.search(p, lower) for p in _STATEMENT_PATTERNS)


def _is_addressed_to_person(text: str) -> bool:
    """True если сообщение начинается с обращения к конкретному лицу (Имя, ...)."""
    if not text or len(text.strip()) < 10:
        return False
    stripped = text.strip()
    # Паттерн: слово (имя) + запятая в начале
    if re.match(r"^[А-Яа-яЁёA-Za-z]+\s*,", stripped):
        return True
    return False


def _is_short_topic_question(text: str) -> bool:
    """Эвристика: короткое сообщение по теме базы без «?» — считаем вопросом."""
    t = (text or "").strip()
    if len(t) > 60 or "?" in t or not t:
        return False
    if _is_statement_or_status(text):
        return False
    lower = t.lower()
    if lower in _NOT_QUESTION_PHRASES or (len(t.split()) <= 2 and lower in ("привет", "готово", "спасибо")):
        return False
...
async def _is_question(message_text: str) -> bool:
    ...
    if _is_statement_or_status(text):
        return False
    if _is_addressed_to_person(text):
        return False
    ...
    try:
        prompt = (
            ...
            "- короткие запросы по теме БЕЗ знака вопроса: «Регламент по открытию магазина», «Документы для начала работы», «Кассовое оборудование» — человек просит информацию из базы, ответь yes.\n"
            "НЕ считай вопросом:\n"
            "- Сообщения о своих действиях: «Сегодня документы перешлю», «Пришлю договор», «Отправил».\n"
            "- Статусы ожидания: «Жду кассовую стойку», «Жду документы».\n"
            "- Короткие утверждения: «Не связывались», «Забрал в понедельник».\n"
            "- Вопросы, обращённые к конкретному человеку: «Андрей, тогда начинаем?», «Сергей, пришлёте документы?».\n"
            "- Запросы контактов в контексте диалога: «Можете контакты направить» (к коллеге).\n"
            "- Приветствия: «Доброго времени», «Как обстановка», «Привет», «Готово», «Спасибо», «Минуту через 5».\n"
            "Если в сообщении есть тема из базы (регламент, документ, касса, открытие, ИП, ЕГАИС и т.п.) и это не приветствие/благодарность/утверждение/статус — ответь yes.\n"
            "Ответь только 'yes' или 'no'.\n\n"
            f"Сообщение: {text}"
        )
        system_content = (
            "Ты помощник. Определяешь, ждёт ли пользователь ответа от базы знаний/поддержки. "
            "Сообщение о своём действии (пришлю, перешлю, жду, забрал) — не вопрос. "
            "Вопрос к конкретному лицу (Имя, …) — не вопрос к базе. "
            "Вопрос может быть без «?». Короткие формулировки по темам регламент, документы, касса, открытие, ИП, ЕГАИС — считай вопросом (yes), если это не утверждение или статус. "
