...
async def _is_question(message_text: str) -> bool:
    ...
    try:
        prompt = (
            ...
            "НЕ считай вопросом: приветствия (привет, добрый день), благодарности (спасибо), «готово», «проверьте», "
            "«мин через 5», короткие согласия, запросы контактов или конкретных людей по имени.\n"
