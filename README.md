...
- **Меню команд как в Telegram** (`setMyCommands`) в MAX **не выставляется** — отдельного аналога в используемом API нет; пользователю доступны текстовые команды и кнопки под сообщениями.
- Остальные сценарии (RAG, `/ask`, рассылки и т.д.) по-прежнему только в Telegram, если не перенесены отдельно.
